#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""RAT QuickDB Python PostgreSQL 缓存性能对比示例

本示例对比启用缓存和未启用缓存的PostgreSQL数据库操作性能差异
使用 PostgreSQL 数据库进行测试

基于 MongoDB 版本的缓存性能对比示例改写为 PostgreSQL 版本
"""
...
class TestUser:
    ...
    def to_json(self) -> str:
        """转换为JSON字符串（包含id，PostgreSQL需要显式指定）"""
        return json.dumps({
            "id": self.id,
            "name": self.name,
            "email": self.email,
...
class PostgreSqlCachePerformanceTest(GracefulShutdownMixin):
    """PostgreSQL缓存性能对比测试"""
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            # 添加带缓存的PostgreSQL数据库
            self._add_cached_postgresql_database()
            
            # 添加不带缓存的PostgreSQL数据库
            self._add_non_cached_postgresql_database()
    ...
    def _create_cached_config(self) -> PyCacheConfig:
        ...
        # L2缓存配置 - 合理的磁盘容量配置
        l2_config = PyL2CacheConfig(f"{self.test_data_dir}/pgsql_cache_test")
    ...
    def _add_cached_postgresql_database(self):
        """添加带缓存的PostgreSQL数据库"""
        cache_config = self._create_cached_config()
        
        response = self.bridge.add_postgresql_database(
            alias="pgsql_cached",
            host="172.16.0.23",
            port=5432,
            database="testdb",
            ...
            ssl_mode="prefer",      # PostgreSQL SSL模式
            cache_config=cache_config
        )
        
        result = json.loads(response)
        if not result.get("success"):
            raise Exception(f"添加缓存PostgreSQL数据库失败: {result.get('error')}")
    
    def _add_non_cached_postgresql_database(self):
        """添加不带缓存的PostgreSQL数据库"""
        # 真正的无缓存配置：不创建任何缓存管理器
        cache_config = None
        
        response = self.bridge.add_postgresql_database(
            alias="pgsql_non_cached",
            host="172.16.0.23",
            port=5432,
            database="testdb",
            username="testdb",
        ...
        result = json.loads(response)
        if not result.get("success"):
            raise Exception(f"添加非缓存PostgreSQL数据库失败: {result.get('error')}")
    ...
    def setup_test_data(self) -> bool:
        ...
        try:
            ...
            # 基础测试用户（为不同数据库使用不同的ID范围避免冲突）
            cached_users = [
                TestUser.new(i, f"缓存用户{i}", f"cached_user{i}@example.com", 20 + (i % 50), 
                           ["北京", "上海", "广州", "深圳", "杭州"][i % 5])
                for i in range(1, 1001)  # ID范围: 1-1000
            ]
            
            non_cached_users = [
                TestUser.new(i, f"非缓存用户{i-1000}", f"non_cached_user{i-1000}@example.com", 20 + ((i-1000) % 50),
                           ["北京", "上海", "广州", "深圳", "杭州"][(i-1000) % 5])
                for i in range(1001, 2001)  # ID范围: 1001-2000，避免与缓存用户ID冲突
            ]
    ...
    def warmup_cache(self) -> bool:
        ...
        try:
            ...
            self.bridge.find(self.table_name, query_conditions_1, "pgsql_cached")
            ...
            self.bridge.find(self.table_name, query_conditions_2, "pgsql_cached")
            ...
            for i in range(1, 21):
                ...
                self.bridge.find(self.table_name, query_conditions, "pgsql_cached")
            ...
            for i in range(1, 11):
                ...
                self.bridge.find(self.table_name, age_conditions, "pgsql_cached")
    ...
    def test_query_operations(self) -> bool:
        ...
        try:
            ...
            start_time = time.time()
            for i in range(1, 101):
                self.bridge.find(self.table_name, query_conditions, "pgsql_non_cached")
    ...
    def test_repeated_queries(self) -> bool:
        ...
        try:
            ...
            # 获取缓存统计信息
            try:
                cache_stats_response = self.bridge.get_cache_stats("pgsql_cached")
    ...
    def test_batch_queries(self) -> bool:
        ...
        try:
            ...
            for i in range(1, 21):
                ...
                self.bridge.find(self.table_name, id_conditions, "pgsql_cached")
            ...
            for i in range(1, 21):
                ...
                self.bridge.find(self.table_name, id_conditions, "pgsql_non_cached")
            ...
            for i in range(1, 21):
                ...
                self.bridge.find(self.table_name, age_conditions, "pgsql_non_cached")
            non_cached_duration = (time.time() - start_time) * 1000  # 转换为毫秒
    ...
    def test_update_operations(self) -> bool:
        ...
        try:
            ...
            for i in range(1, 21):
                ...
                self.bridge.update(self.table_name, update_conditions, update_data, "pgsql_cached")
            cached_duration = (time.time() - start_time) * 1000  # 转换为毫秒
            ...
            for i in range(1, 21):
                ...
                self.bridge.update(self.table_name, update_conditions, update_data, "pgsql_non_cached")
            non_cached_duration = (time.time() - start_time) * 1000  # 转换为毫秒
            ...
            if self.bridge:
                try:
                    ...
                    if time.time() - cleanup_start < cleanup_timeout:
                        ...
                        self.bridge.delete(self.table_name, delete_conditions, "pgsql_cached")
...
def main():
    ...
    test_instance = PostgreSqlCachePerformanceTest()
